module Bookshop =
    ...
    let private prepareLines catalog inventory (lines: CartLine list) =
        ...
        |> List.fold
            (fun state line ->
                result {
                    let! priced, currentInventory = state
                    let! nextLine, nextInventory = prepareLine catalog currentInventory line
                    return nextLine :: priced, nextInventory
                })
            (Ok([], inventory))
    ...
    let placeOrder shippingMethod (cart: Cart) (state: State) =
        ...
        | Some customer ->
            result {
                let! lines, inventory = prepareLines state.Catalog state.Inventory cart.Lines
